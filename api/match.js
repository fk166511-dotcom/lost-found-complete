...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') return res.status(200).json({ status: 'OK', message: 'Lost & Found API ready!' });
  ...
  if (path.endsWith('/upload')) {
    try {
      const { image_base64, folder } = body;
  ...
  if (path.endsWith('/match')) {
    try {
      const { img1_url, img2_url, img1_base64, img2_base64 } = body;
      ...
      } else if (img1_base64 && img2_base64) {
  ...
  return res.status(200).json({ status: 'OK', routes: ['/api/match', '/api/upload'] });
  }
